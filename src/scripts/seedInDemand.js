...
async function seed() {
    ...
    for (const job of jobsData) {
        ...
        // Daily routines
        await db.insert(schema.dailyRoutines).values(job.dailyRoutines.map((text) => ({
            jobId: insertedJob.id,
            text,
        })));
        // Skills
        await db.insert(schema.jobSkills).values(job.skills.map((s) => ({
            jobId: insertedJob.id,
            skill: s.skill,
            priority: s.priority,
        })));
        // Career path
        await db.insert(schema.careerPaths).values(job.career.map((c) => ({
            jobId: insertedJob.id,
            level: c.level,
