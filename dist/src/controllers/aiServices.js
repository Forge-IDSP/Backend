...
class AiService {
    constructor(apiKey) {
        this._gemini = new genai_1.GoogleGenAI({ apiKey });
    }
    async aiResponse(userResponse, chatHistory, step) {
        const conversationHistory = chatHistory
            .map((m) => `${m.content.type}: ${m.content.text}`)
            .join("\n");
        const mostRecentQuestion = chatHistory[chatHistory.length - 2];
        const { isReady, isProgressionQuestion } = await this.checkNextPoint(userResponse, mostRecentQuestion.content.text, step);
        ...
        return await this.handleProgression(userResponse, conversationHistory, step, {
            isReady,
            isProgressionQuestion,
        });
    }
    //Done
    async getCareerData(data) {
            ...
            config: {
                ...
                responseSchema: {
                    ...
                    properties: {
                        careerName: {
                            type: "string",
                            description: "Name of the recommended trade career, e.g. 'Electrician'",
                        },
                        summary: {
                            type: "string",
                            description: "One-sentence simple summary of why this career fits the user (grade 9-12 level).",
                        },
                        reasons: {
                            type: "array",
                            items: {
                            ...
                            description: "Array of 3-4 specific reasons why this career matches the student's quiz answers (interests, environment, work style, priority). Each reason should be a complete sentence written at grade 9-12 level.",
                        },
                    },
    ...
    async initializeCareerPath(trade) {
        const aiRaw = await this._gemini.models.generateContent({
            model: "gemini-2.5-flash",
            contents: `
    ...
    async getApprenticeLevels(careerName) {
        const prompt = `
  ...
  Be specific to ${careerName} trade practices in British Columbia.`,
                            ...
                            items: {
                                ...
                                properties: {
                                    ...
                                    items: {
                                        ...
                                        description: "3-4 specific skills or tasks for this level, relevant to BC",
                                    },
                                },
                                required: ["title", "items"],
                            },
                            description: "Array of 4 apprentice levels following ITA BC structure",
                        },
                    },
                    required: ["levels"],
                },
    ...
    async checkNextPoint(userResponse, currentQuestion, step) {
        try {
                ...
                config: {
                    ...
                    responseSchema: {
                        ...
                        properties: {
                            isProgressionQuestion: {
                                type: "boolean",
                                description: "Whether Anna is asking if user wants to move to next stage",
                            },
                            isReady: {
                                type: "boolean",
                                description: "Whether user indicates readiness (only valid if isProgressionQuestion is true)",
                            },
                        },
    ...
    async handleProgression(userText, chatHistory, step, context) {
        let contextPrompt;
...
exports.aiService = new AiService(process.env.GEMINI_API_KEY);
